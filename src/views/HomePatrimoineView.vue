    ...
    <div class="h-screen w-screen">
        ...
        <div  class="w-full ">
            <!-- the side bar -->
             ...
             <!-- The main content -->
             <div class=" h-full pl-12 pt-9" v-if="loaded">
                 <div class=" heroImg flex justify-center pt-11 pb-9">
                    <div>
                        <p class=" text-white font-bold text-xl sm:text-5xl font-playfair text-center"> Les demandes nécessitant votre autorisation </p>
                        <p class="  text-center pt-4 text-white font-poltawski ">Veuillez trouver ci-joint l'ensemble des demandes qui requièrent votre autorisation.</p>
                    </div>                                                                                                                                                                                                          

                </div>
                 <div class="h-full w-full p-2 flex flex-col gap-2 items-center justify-center relative bg-gradient-to-b from-slate-50 to-slate-300" v-if="dataLoaded">
                    <!---div holding button de map-->
                   <div class="w-full  flex items-center justify-end px-5 gap-2">
                    <!-- button de map  --->
                    <!-- <router-link to="/createDemande"> -->
                        <button @click="viewINMap('all')" type="button" class=" transition-all duration-300 group  gap-5 text-white bg-[#2557D6] hover:bg-white hover:text-black hover:ring hover:ring-blue-600  focus:outline-none font-poppins rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center">
                            Visualiser la localisation des demandes
                       <svg class="fill-white group-hover:fill-black" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path d="M12 2C7.589 2 4 5.589 4 9.995 3.971 16.44 11.696 21.784 12 22c0 0 8.029-5.56 8-12 0-4.411-3.589-8-8-8zm0 12c-2.21 0-4-1.79-4-4s1.79-4 4-4 4 1.79 4 4-1.79 4-4 4z"></path></svg>                        </button>
                  <!--   </router-link> -->
<!-- end of button de map -->
                    
                   </div>
                    <!--- end of div holding button de map-->
                       <!---  div holding all demmandes -->
                   <div :class="`${areYouSure || view || writing || mapUp  ?'blur-xl':''} font-poppins  w-full h-full p-10 overflow-x-hidden flex flex-wrap items-center justify-center gap-5`  ">
 <!--                
 -->
 <div v-for="(cmd,i) in commandes " :key="cmd.id" class=" bg-white drop-shadow-2xl  w-96 h-60  border border-black rounded-lg flex flex-col  card">
  ...
  <path stroke-linecap="round" stroke-linejoin="round" d="M7.5 3.75H6A2.25 2.25 0 003.75 6v1.5M16.5 3.75H18A2.25 2.25 0 0120.25 6v1.5m0 9V18A2.25 2.25 0 0118 20.25h-1.5m-9 0H6A2.25 2.25 0 013.75 18v-1.5M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
        ...
        <!----center -->
        <div  class="options   flex flex-col justify-start font-poppins items-center gap-1 h-full   w-1/2 p-1 overflow-hidden">
            <div v-if="options[i]"   class=" w-full  flex items-center justify-center  ">
                <button  @click="authorize(cmd)" class="bg-stone-300 p-2  rounded-lg w-full flex justify-between items-center  font-poppins font-bold ">
                    accepter
                    <svg class="w-6 h-6 fill-green-400 " xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M19 3H5c-1.103 0-2 .897-2 2v14c0 1.103.897 2 2 2h14c1.103 0 2-.897 2-2V5c0-1.103-.897-2-2-2zm-7.933 13.481-3.774-3.774 1.414-1.414 2.226 2.226 4.299-5.159 1.537 1.28-5.702 6.841z"></path></svg>
                </button>
            </div>
            <div v-if="options[i]"   class=" w-full  flex items-center justify-center ">
                <button @click="popUpAreYouSure(cmd)" class="bg-stone-300 p-2  rounded-lg w-full flex justify-between items-center  font-poppins font-bold ">
                    refuser
                    <svg class="w-8 h-8 fill-red-400 " xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M17 5H7a2 2 0 0 0-2 2v10a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V7a2 2 0 0 0-2-2zm-1 8H8v-2h8z"></path></svg>
                </button>
            </div>
            <div v-if="options[i]"   class=" w-full  flex items-center justify-center ">
                <button @click="viewINMap(cmd)" class="bg-stone-300 p-2  rounded-lg w-full flex justify-between items-center  font-poppins font-bold ">
                    dans la carte
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-8 h-8 fill-indigo-400 " viewBox="0 0 24 24" ><path d="m9 6.882-7-3.5v13.236l7 3.5 6-3 7 3.5V7.382l-7-3.5-6 3zM15 15l-6 3V9l6-3v9z"></path></svg>                </button>
            </div>
    ...
    <!-- end of btns options ---->
                   ...
                   <div  :class="`${areYouSure?'scale-100':'scale-0'} transition-all duration-300  w-full lg:w-1/2  bg-white   absolute p-5 flex flex-col gap-5 items-center justify-center ring-2 ring-indigo-500 rounded-lg`">
                    <div class="w-full flex justify-end items-center   px-10 ">
                            <svg  @click="notAuthorize(cmd,false)"  class="w-10 h-10 fill-blue-500 cursor-pointer" xmlns="http://www.w3.org/2000/svg"  viewBox="0 0 24 24"><path d="M20 3H4c-1.103 0-2 .897-2 2v14c0 1.103.897 2 2 2h16c1.103 0 2-.897 2-2V5c0-1.103-.897-2-2-2zM4 19V7h16l.001 12H4z"></path><path d="m15.707 10.707-1.414-1.414L12 11.586 9.707 9.293l-1.414 1.414L10.586 13l-2.293 2.293 1.414 1.414L12 14.414l2.293 2.293 1.414-1.414L13.414 13z"></path></svg>
                        </div>
         ...
         <button @click="notAuthorize(cmd,true)" class=" group relative w-full lg:w-1/2 bg-white ring-1 ring-emerald-400 rounded-md gap-2 p-2 text-center flex justify-center items-center font-poppins font-semibold">
        ...
        <!--  <button @click="notAuthorize(cmd,false)" class=" group relative w-full lg:w-1/2 bg-white ring-1 ring-red-400 rounded-md gap-2 p-2 text-center flex justify-center items-center font-poppins font-semibold">
          ...
          no annuler
                   ...
                   <!--- end of sure pop up -->
                   <!-- - text editor --->
                   <div v-if="writing" class="absolute top-0 left-1/2 -translate-x-1/2 translate-y-1/3  p-1  font-poppins font-bold  rounde-lg cursor-pointer ">
  <div class="w-full  flex p-1 justify-center items-cente">
    <svg @click="writing=false" class="w-8 h-8 fill-blue-500 hover:scale-110 hover:fill-red-500  hover:bg-blue-500 transition-all duration-300 bg-red-500 rounded-full" xmlns="http://www.w3.org/2000/svg"  viewBox="0 0 24 24"><path d="m16.192 6.344-4.243 4.242-4.242-4.242-1.414 1.414L10.535 12l-4.242 4.242 1.414 1.414 4.242-4.242 4.243 4.242 1.414-1.414L13.364 12l4.242-4.242z"></path></svg>
  </div>
  ...
  <div  class="w-full py-2 flex justify-center items-center  ">
    <button @click="envoyerAuthorisation" class=" envoyer bg-blue-500 w-full p-2 rounded-sm hover:bg-white hover:text-black text-white hover:ring hover:ring-blue-300 shadow-xl"> confirmer</button>
                   ...
                   <!-- end of text editor -->
                   <!-----map view-->
                   <div  :class="`${mapUp ?'scale-100':'scale-0'}  h-full transition-all  duration-500  scale-1 border-2 border-neutral-600   overflow-y-auto z-50 absolute top-0 left-0 w-full  bg-white  flex flex-col gap-2 items-center justify-center rounded-lg`">
                        <div class="w-full flex justify-end items-center   px-10 ">
                            <svg @click="mapUp=false" class="w-10 h-10 fill-blue-500 cursor-pointer" xmlns="http://www.w3.org/2000/svg"  viewBox="0 0 24 24"><path d="M20 3H4c-1.103 0-2 .897-2 2v14c0 1.103.897 2 2 2h16c1.103 0 2-.897 2-2V5c0-1.103-.897-2-2-2zM4 19V7h16l.001 12H4z"></path><path d="m15.707 10.707-1.414-1.414L12 11.586 9.707 9.293l-1.414 1.414L10.586 13l-2.293 2.293 1.414 1.414L12 14.414l2.293 2.293 1.414-1.414L13.414 13z"></path></svg>
                        </div>
                        <div v-if="mapUp" class="w-full h-full bg-red-500">
                            <Map :coords="mappers" />
                        </div>
                 </div>
                   <!----- end map view-->
...
import patriService from '../Services/patriService'
 ...
 */
        ...
        */
        ...
        const now = new Date() 
        const user = ref({})
        const states=ref(['enregister','envoyer','en cours','accepter']);
...
/******* */
        ...
        onMounted(async () => {
            ...
            commandes=await patriService.getAlldemandes();
            console.log(commandes)
            commandes=commandes.filter(c=>{
                for( let com of c.commentaires){
                    if(com.sujet.includes('patrimoine'))
                    return
                }
                return c;
            })
...
let notAuthorize=(cmd,answer)=>{
  ...
  if(answer){   
    ...
    console.log(cmd)
    patriService.refuseDemand(comment.value,choosedDemande.value,user.value,router);
...
let viewINMap=(cmd)=>{
    console.log(cmd)
  if(cmd==="all"){
    mappers.value=commandes.map(x=>{return {ref_numero:x.ref_numero,localisation:{adresse:x.localisation, latitude:x.latitude,longitude:x.longitude}}})
       
  }
    else{ 
        mappers.value=[{ref_numero:cmd.ref_numero,localisation:{adresse:cmd.localisation, latitude:cmd.latitude,longitude:cmd.longitude}}]
...
let envoyerAuthorisation=async()=>{
  ...
  .to('.envoyer',.05,{xPercent:0})
  .call(async ()=>{
    await patriService.acceptDemand(form,choosedDemande.value,user.value,router);
  })
  }
else {
    gsap.timeline()
