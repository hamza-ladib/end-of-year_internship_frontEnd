        ...
        <div class="">
            ...
            <div class=" h-full pl-12 pt-9">
                <div class="heroImg flex justify-center pt-11 pb-9">
                    <div>
                        <p class=" text-white font-bold text-xl sm:text-5xl font-playfair text-center">Prise de décision
                            pour la demande</p>
                        <p class=" text-center pt-4 text-white font-poltawski text-sm">Nous vous invitons à prendre une
                            décision concernant la demande en cours .</p>
                    </div>
                    ...
                    <div class="mt-9">
                        ...
                        <transition name="fade">
                            <div class=" bg-green-800 rounded-lg  flex justify-between items-center mt-4 px-4 mx-24 py-6 border-green-400 border-2"
                                ...
                                <p class="text-green-400 font-bold text-lg">
                                    Votre Décision a été enregistré avec succès.</p>
                        ...
                        <transition name="fade">
                            <div class=" mt-9  mx-12 bg-gray-300 pt-8 rounded-md" v-if="showPopUp">
                                <p class=" font-bold text-lg text-center text-gray-700">Vous êtes sur le point d'enregistrer
                                    votre observation comme décision finale, êtes-vous d'accord ?</p>
                        ...
                        <transition name="fade">
                            <div class=" mt-9" v-if="toggleForm">
                                <p class=" text-lg font-playfair mx-4 pr-1 w-fit relative group ">Il est temps d'ajouter
                                    votre décision pour la demande en cours
                                <p
                                ...
                                <div class=" py-6">
                                    <form @submit.prevent="submitForm" class=" grid">
                                        <label class=" font-poppins text-base  md:text-lg ">Décision</label>
                                        <input type="text"
                                            class=" mt-2 py-1 border-2 rounded-md border-gray-200 outline-none px-2"
                                            placeholder="Veuillez saisir votre décision: favorable, non favorable ou dérogation" required
                                            v-model="inputData.sujet">
                                        <label class=" font-poppins text-base  md:text-lg  mt-3">Message</label>
                                        <textarea
                                            class=" mt-2 py-1 border-2 rounded-md border-gray-200 outline-none px-2 h-20"
                                            placeholder="Veuillez entrez votre décision final pour cette demande" required
                                            v-model="inputData.message"></textarea>
                                        <button
...
import { accessToken_state, isUserAuth, getSingleDemande, updateFeedBack } from '../Services/service'
...
import footerComponent from '../components/footerComponent.vue'
        ...
        onMounted(async () => {
            ...
            if (response == true) {
                ...
                demandeID = router.currentRoute.value.params.id;
            
                informations.value = await getSingleDemande(demandeID)
        ...
        const sendData = async () => {
            ...
            console.log(inputData.idUser,'xxx',user.value.id)
            inputData.idUser = user.value.id;
            const response = await updateFeedBack(demandeID, inputData)
...
<!--
                                ...
                                <p class=" w-fit px-2 py-1 border-b-2 border-gray-300 font-poppins font-bold "> N° <span class="text-blue-700">8965</span></p>
                                <p class=" px-2 py-1 border-b-2 border-gray-300 font-poppins font-bold bg-red-400">
                                    It is a long established fact that a reader will of a page when looking at its layout.
                                </p>
                                <p class=" px-2 py-1 border-b-2 border-gray-300 font-poppins font-bold ">19 <span class=" text-sm text-gray-400">(Mois) </span></p>    
                                </div>
