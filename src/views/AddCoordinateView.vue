        ...
        <div class="">
            ...
            <div class=" h-full pl-12 pt-9">
                <div class="heroImg flex justify-center pt-11 pb-9">
                    <div>
                        <p class=" text-white font-bold text-xl sm:text-5xl font-playfair text-center">L'ajout des coordonnées</p>
                        <p class=" text-center pt-4 text-white font-poltawski text-sm">Nous vous prions d'inclure les coordonnées de longitude et de latitude pour cette demande.</p>
                    </div>
                        ...
                        <transition name="fade">
                            <div class=" bg-green-800 rounded-lg  flex justify-between items-center mt-4 px-4 mx-24 py-6 border-green-400 border-2"
                                ...
                                <p class="text-green-400 font-bold text-lg">
                                    Vos coordonnées ont été ajouté avec succès.</p>
                        ...
                        <transition name="fade">
                            <div class=" mt-9" v-if="toggleForm">
                                <p class=" text-lg font-playfair mx-4 pr-1 w-fit relative group ">Il est temps d'ajouter
                                    les coordonnées pour la demande en cours
                                <p
                                ...
                                <div class=" py-6">
                                    <form @submit.prevent="submitForm" class=" grid">
                                        <label class=" font-poppins text-base  md:text-lg ">Longitude</label>
                                        <input type="text"
                                            class=" mt-2 py-1 border-2 rounded-md border-gray-200 outline-none px-2"
                                            placeholder="Veuillez saisir le longitude de la demande" required
                                            v-model="inputData.longitude" />
                                        <label class=" font-poppins text-base  md:text-lg  mt-3">Latitude</label>
                                        <input type="text"
                                            class=" mt-2 py-1 border-2 rounded-md border-gray-200 outline-none px-2"
                                            placeholder="Veuillez saisir le latitude de la demande" required
...
import { accessToken_state, isUserAuth, getSingleDemande, addFeedBack,addCoordinationService } from '../Services/service'
...
export default {
    ...
    setup (props) {
        ...
        const sendData = async () => {
            console.log(idDemand.value,router.currentRoute.value.params.id)

            const response = await addCoordinationService(idDemand.value==NaN?router.currentRoute.value.params.id:idDemand.value, inputData.value)
        ...
        return {user, logOutFunction, showError, hideAlert, toggleForm, submitForm, showPopUp, sendData, informations, dataLoaded, dateDebut, inputData}
    }
}
</script>
