        ...
        <div class="">
            ...
            <div class=" h-full pl-12 pt-9">
                ...
                <div class="heroImg flex justify-center pt-11 pb-9">
                    <div>
                    <p class=" text-white font-bold text-xl sm:text-5xl font-playfair text-center">Créer votre demande</p>
                    <p class="  text-center pt-4 text-white font-poltawski">Merci de compléter le formulaire pour générer votre
                        demande.</p>
                    </div>
                ...
                <div class="drop-shadow-xl bg-gradient-to-b from-slate-50 to-slate-200  py-4">
                    ...
                    <div class="  flex justify-center  pt-9 pb-4">
                        <!--
                    ...
                    -->
                    <FormDemande :formInputs='formInputs' :submitForm='submitForm' :onFileInputChange='onFileInputChange' />
                    </div>
                </div>
            </div>
        </div>
...
import { accessToken_state, isUserAuth, saveDemandUser, archiveDemandUser } from '../Services/service'
...
export default {
    ...
    setup() {
        ...
        const enregistreDemande = async () => {
            ...
            if (answer) {
                const response = await saveDemandUser(userId,form)
                if (response.state) {
        ...
        const archiveDemande = async () => {
            const response = await archiveDemandUser(userId,form)
            if (response.state) {
                saveSuccess.value = await response.message
                showPopUp.value = false
                router.push('/home')
        ...
        return { user, logOutFunction, formInputs, submitForm, onFileInputChange, showPopUp, enregistreDemande, archiveDemande, error, saveSuccess }
    }
}
</script>
